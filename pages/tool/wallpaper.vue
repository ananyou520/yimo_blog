		...
		<view v-if="dataList.length != 0" class="bg-white" style="padding: 25rpx;">
			<view class="grid margin-bottom col-2">
				<view @click="lookImg(item)" v-for="(item,index) in dataList" :key="index" class="grid-box">
					<view style="width: 100%;" v-if="item.ad == 1">
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			//获取文章类型标签
			getDicts("app_wallpaper_label").then(response => {
				this.TypeList = [];
		...
		methods: {
			lookImg(itme){
				this.$tab.navigateTo('/pages/tool/look_img?zipUrl='+itme.compressImgUrl+"&url="+itme.imgUrl+"&id="+itme.id+"&isAd="+itme.isAd+"&type=1")
			},
			searchType(type) {
...
<style>
	...
	.grid-box {
		padding: 7rpx;
		position: relative;
		display: inline-block; /* 使得容器与其他内容在同一行 */
	}
